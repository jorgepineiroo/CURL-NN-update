...
def prepare_optimizer(name, net, lr, min_lr, train_samples, batch, epochs, warmup, gamma, wd,
    ...
    # Create epoch-based schedulers (more stable than step-based)
    # Warmup phase: linear increase from start_factor*lr to lr over warmup epochs
    warmup_epochs = int(warmup)
    warmup_scheduler = LinearLR(optimizer, start_factor=0.1, total_iters=warmup_epochs)
    
    # Cosine annealing phase: from lr to min_lr over remaining epochs
    cosine_epochs = epochs - warmup_epochs
    cosine_scheduler = CosineAnnealingLR(optimizer, T_max=cosine_epochs, eta_min=min_lr)
    
    # Combine both schedulers (milestone is now in epochs, not steps)
    lr_scheduler = SequentialLR(optimizer, 
                                schedulers=[warmup_scheduler, cosine_scheduler],
                                milestones=[warmup_epochs])
    
    logging.info(f'select cosine lr scheduler with {warmup} epochs warmup (epoch-based)')
    ...
    def compute_ssim(image_batchA, image_batchB):
        ...
        for i in range(0, num_images):
            ...
            imageB = ImageProcessing.swapimdims_3HW_HW3(
                image_batchB[i, 0:3, :, :])
            ssim_val += ssim(imageA, imageB, data_range=imageA.max() - imageA.min(), channel_axis=-1,
