...
class NEW_CURLLayer(nn.Module):
    ...
    def forward(self, x, device):
        ...
        # img_residual_new = torch.clamp(ImageProcessing.hsv_to_rgb_new(img_hsv, device), 0, 1)
        ...
        # print(gradient_regulariser_rgb.shape,gradient_regulariser_hsv.shape,gradient_regulariser_lab.shape)
        # exit(9991)
...
class CURLNet(nn.Module):
    ...
    def forward(self, img):
        ...
        img, gradient_regulariser = self.curllayer(feat, self.device)
        return img, gradient_regulariser
