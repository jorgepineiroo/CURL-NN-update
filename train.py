    ...
    for epoch in range(num_epoch):
        ...
        for batch_num, (input_img_batch, gt_img_batch, _) in enumerate(training_dataloader):
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                logging.warning(f"NaN or Inf loss detected at epoch {epoch+1}, batch {batch_num}. Skipping batch.")
                continue
            ...
            if (batch_num + 1) % 10 == 0:
                train_writer.add_scalar('step_loss', loss.item(), epoch * num_batches + batch_num)

        end = time.time()
        logging.info(f'epoch %d: {int(end - start)}s - train_loss: %.8f' % (epoch + 1, running_loss / num_batches))

        # Step learning rate scheduler once per epoch (more stable than per batch)
        lr_scheduler.step()
        train_writer.add_scalar('learning_rate', lr_scheduler.get_last_lr()[0], epoch + 1)
